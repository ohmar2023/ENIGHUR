...
```{r}
df_insumos_enighur <- read_excel("INSUMOS-ENIGHUR-NUESTRA.xlsx", sheet = "INSUMOS")
```
...
f_optimizacion_mer_dominios <- function(mer_param){
  ...
  str_dominios <- df_insumos_enighur$DOMINIO[1:33] # todos los dominios - 33
...
```{r, warning=FALSE, message=FALSE}
grilla_dominio <- seq(0.03, 0.07, by = 0.001) # se fija un vector de los posibles mer
...
names(vctr_optim_Dominio) <- grilla_Dominio 
solucion_Dominio <- names(vctr_optim_Dominio[abs(vctr_optim_Dominio) <= 5]) %>% as.numeric()
solucion_Dominio <- min(solucion_Dominio)
solucion_Dominio
```
...
df_margen_error <- data.frame(DOMINIO = c(ciudades_autorepresentadas, resto,
                              ...
                              mer_deseado = c(rep(0.0433, length(ciudades_autorepresentadas)),
                                              rep(0.0433*1.5, length(resto)),
                                              rep(0.0433*3, length(amazonia)))
                                              )
df_insumos_enighur_region <- df_insumos_enighur %>% slice(1:33) %>% 
