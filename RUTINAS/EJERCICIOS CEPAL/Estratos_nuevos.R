...
upm_prov <- Marco %>%  mutate(cod_6 = substr(id_conglomerado,1,6)) %>% 
  filter(!cod_6 %in% v_ciudades_auto) %>% 
  group_by(dom) %>% summarise(n=n())

upm_ciudades_auto <- Marco %>% mutate(cod_6 = substr(id_conglomerado,1,6)) %>% 
  filter(cod_6 %in% v_ciudades_auto) %>% group_by(cod_6) %>% 
  summarise(n=n()) %>% rename("dom"=cod_6)

upm_dom <- rbind(upm_prov,upm_ciudades_auto)
upm_dom$dom <- c("1_p","2_p","3_p","4_p","5_p","6_p","7_p","8_p","9_p","10_p","11_p","12_p",
...
prom_hogares_upm <- base %>% filter(!is.na(d1)) %>% 
...
#aux <- base_estrato %>% group_by(provincia,area,estrato_f,id_6) %>% summarise(n()) %>% View()
#table(base$area,base$zona,useNA = "ifany") %>% View()
...
for (i in c(1:12))
{
  ...
  ind$nombre_dom <- nombres_dom <- c("Azuay",
                                     "Bolívar",
                                     "Cañar",
                                     "Carchi",
                                     "Cotopaxi",
...
saveWorkbook(wb,"INSUMOS_12.xlsx",overwrite = T)
